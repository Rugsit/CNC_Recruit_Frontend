...
export default function FileUpload({ title, description, accept, register, errorMessage, icon, setValue, trigger, existedFileName }: FileUploadProps) {
    ...
    const displayFileName = existedFileName ? existedFileName : selectedFile?.name;
            ...
            }
            {
                ...
                (<div onClick={handlePreview} style={{ cursor: 'pointer' }} className="flex flex-row gap-x-5 py-3 px-4 items-center border border-gray-200 rounded-lg">
                    <Image
                        ...
                        alt="file icon"
                    />
                    <p className="text-base text-black" onClick={handlePreview} style={{ cursor: 'pointer' }}>{displayFileName}</p>
                </div>)
            }
        </section>
    );

...
export default function QuestionCard({
  ...
  return (
    <article className='bg-white max-w-[750px] mx-auto mb-12 rounded-xl shadow-md min-[440px]:p-5 max-[440px]:p-3 font-medium relative'>
      <Button
        isIconOnly
        className='absolute top-5 right-5 bg-transparent rounded-full'
        onClick={() => {
          deleteQuestion();
        }}
      >
      ...
      <div className='flex gap-5 justify-start items-center max-[440px]:flex-col'>
        ...
        <div
          className={clsx('border-[1px] py-2 px-9 rounded-xl ', {
      ...
      <div className='text-white flex gap-3 mt-7'>
        <button className='py-2 px-4 cursor-default bg-primary-400 rounded-xl'>
          {questionNisit.question_type === 'knowledge' ? 'ความรู้' : 'ทัศนคติ'}
        </button>
        <button
          className={clsx('py-2 px-4 cursor-default  rounded-xl', {
        ...
        >
          {questionNisit.question_difficulty === 'hard'
            ? 'ยาก'
            : questionNisit.question_difficulty === 'normal'
              ? 'ปานกลาง'
              : 'ง่าย'}
        </button>
      </div>
      ...
      <div className='flex justify-between w-full'>
        ...
        <button
          className={clsx(
            'transition-transform focus:outline-none border-none',
            {
      ...
      <div
        className={clsx('transition-all grid ', {
      ...
      >
        <div className='overflow-hidden mt-4'>
          <p className='leading-7 text-[#3B434F] font-normal'>
            <span className='font-bold mr-4 text-[#3B434F]'>
              คำตอบที่คาดหวัง:
            </span>
          ...
          />
          <p
            className={clsx('my-8 leading-7 font-normal text-[#3B434F]', {
              ' fixed opacity-0': editGrade,
            })}
          >
            <span className='font-bold mr-4 '>ความคิดเห็น:</span>
            {questionNisit.comment === ''
              ? 'ไม่มีความคิดเห็น'
              : questionNisit.comment}
          </p>
          <div className='mt-10 '>
            ...
            <div
              className={clsx(
                'flex justify-center items-center w-full max-[440px]:gap-1 min-[440px]:gap-5 mt-10',
                {
            ...
            >
              ...
              >
                <p className='absolute -top-7 text-zinc-400 text-nowrap'>
                  ผ่าน
                </p>
              </button>
            </div>
